...
app.get('/api', (req, res) => {
  res.json({
    name: 'UX Collective API Service',
    status: 'running',
    version: '1.0.0',
    documentation: {
...
app.post('/api/ping-db', async (req, res) => {
  const { source = 'React Web App', message = 'Interactive connection test' } = req.body || {};
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 UXCO Node.js server running on http://0.0.0.0:${PORT}`);
});
